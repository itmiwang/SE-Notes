...
当 `compareChangedGroup` 方法返回的 `List<ConfigGroupEnum>` 非空时，调用 `generateResponse` 
...
以选择器 `selector` 为例，当数据发生变化时，调用 `afterSelectorChanged` 方法，其他 `appAuth` 、`metaData` 、`plugin` 、`rule` 类似。
